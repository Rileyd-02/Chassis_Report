...
It will map the 'LatestSubChassis' from the reference table to your planning sheet based on 'Style'.

**Instructions:**
1. Upload your dynamic planning Excel file and the Subchassis reference table.
2. The tool will try to identify the 'Style' column using fuzzy matching.
3. You can confirm or adjust the detected column.
...
if uploaded_dynamic and uploaded_reference:
    try:
        ...
        if st.button("Map Subchassis"):
            ...
            # Merge on Style only
            merged_df = pd.merge(dynamic_df, reference_df[['Style', 'LatestSubChassis']], 
                                 left_on=style_col, right_on='Style', how='left')
            merged_df.drop(columns=['Style'], inplace=True)
